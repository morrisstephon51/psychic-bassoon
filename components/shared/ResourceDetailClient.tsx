...
import { Lock, Download, CheckCircle, BookOpen, ArrowLeft, AlertCircle } from 'lucide-react'
...
export default function ResourceDetailClient({ resource }: { resource: Resource }) {
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    // Only unlock on a confirmed submission. Unlocking on failure would hide
    // the gate while silently dropping the lead (e.g. a missing/misconfigured
    // NEXT_PUBLIC_FORMSPREE_NEWSLETTER_ID makes submitToFormspree return
    // { ok: false }) — the visitor thinks they subscribed but no email is ever
    // captured. Surface the error and let them retry instead.
    if (result.ok) {
      setUnlocked(true)
      setStatus('success')
    } else {
          ...
          <div className="max-w-4xl mx-auto">
            ...
            >
              ...
              <form onSubmit={handleUnlock} className="space-y-3">
                ...
                {status === 'error' && (
                  <p role="alert" className="flex items-start gap-1.5 text-red-600 text-xs text-left">
                    <AlertCircle size={14} className="flex-shrink-0 mt-0.5" />
                    <span>
                      Something went wrong and we couldn&apos;t unlock this. Please try again, or email
                      us at{' '}
